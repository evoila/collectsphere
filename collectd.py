...
class Values(PluginData):
    ...
    def dispatch(self, time=None, type=None, type_instance=None, values=None, meta=None):
        ...
        if not meta:
            meta = self.meta
        print("host=%s, plugin=%s, plugin_instance=%s, time=%s, type=%s, type_instance=%s, values=%s, meta=%s" % (
        self.host, self.plugin, self.plugin_instance, time, type, type_instance, values, meta))
